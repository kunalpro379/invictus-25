...
export default function UpdateProfile() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const userData = await fetchUserData(token);
      await axios.put("http://localhost:3000/api/v1/users/update", {
        userId: userData?.id || "",
        shortBio: data.shortBio,
        interests: Array.isArray(data.interests) ? data.interests.join(", ") : data.interests, // Convert back to string
        ...
        <CardContent className="p-6 space-y-6">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="space-y-4">
              <div>
                <Label className="text-gray-700">Short Bio</Label>
                <Textarea placeholder="Tell us about yourself..." {...register("shortBio")} className="border-gray-300 focus:ring-blue-500 focus:border-blue-500" />
              ...
              <div>
                ...
                <Input type="text" placeholder="e.g., AI, Machine Learning" {...register("interests")} className="border-gray-300 focus:ring-blue-500 focus:border-blue-500" />
              ...
              <div>
                ...
                <Input type="text" placeholder="Your institution" {...register("instituteName")} className="border-gray-300 focus:ring-blue-500 focus:border-blue-500" />
                {errors.instituteName && <p className="text-sm text-red-500">{errors.instituteName.message}</p>}
              </div>

              <div>
                <Label className="text-gray-700">Research Papers</Label>
                {papers.map((paper, index) => (
                  <div key={index} className="flex gap-2 items-center">
                    <Input placeholder="Paper Name" value={paper.name} onChange={(e) => handlePaperChange(index, "name", e.target.value)} className="border-gray-300" />
                    <Input placeholder="Paper URL" value={paper.url} onChange={(e) => handlePaperChange(index, "url", e.target.value)} className="border-gray-300" />
                    <Button type="button" variant="ghost" size="sm" onClick={() => deletePaper(index)}>
                      <Trash2 size={16} />
                    </Button>
                  </div>
                ))}
                <Button type="button" onClick={addPaper} className="mt-2 bg-blue-500 text-white hover:bg-blue-600">
                  Add Paper
                </Button>
              </div>
